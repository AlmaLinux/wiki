...
# AlmaLinux 10.3 Beta Release Notes

These are the release notes for AlmaLinux OS 10.3 Beta for all supported architectures:
...
Distributed kernel version: 6.12.0-264.el10
...
**ModSecurity is back.** The `mod_security` web application firewall and the OWASP Core Rule Set (`mod_security_crs`) return to Enterprise Linux 10. They shipped in AlmaLinux 8 and 9 but were absent from 10.0 through 10.2. This is a large jump: AlmaLinux 9 carries ModSecurity 2.9.6 with CRS 3.3.5, while 10.3 Beta ships 2.9.14 with CRS 4.22.0. CRS 4 is a rework of the rule set rather than a routine update, so rules and tuning carried over from the CRS 3.3 series will need reviewing.

**Podman 6, and container configuration moves out of `/etc`.** The container stack moves as a unit — podman 6.1.0, buildah 1.45.0, skopeo 1.24.0, crun 1.29.1, and netavark and aardvark-dns 2.1.0 — and `containers-common` 6.0 relocates the vendor defaults from `/etc/containers` to `/usr/share/containers`, adding vendor drop-in configuration for `containers.conf`, `storage.conf` and `registries.conf`. Configuration you placed under `/etc/containers` is preserved across the upgrade by a `%posttrans` scriptlet and still takes precedence over the vendor defaults. Rootful and rootless storage defaults are now separate drop-ins. Because the pieces have to move together, `containers-common` declares hard conflicts against `podman < 6`, `buildah < 1.44` and `skopeo < 1.23`; netavark and aardvark-dns also carry an epoch bump to keep the upgrade path from AlmaLinux 9 intact.

- Compiler updates:
  - GCC Toolset 16 (gcc 16.1.1, binutils 2.46)
  - LLVM Toolset 22.1.8
  - Rust Toolset 1.97.1
  - GCC 14.4.1
...
- Added new packages:
  - Node.js 26.1.0
  - .NET 11 (preview)
  - ModSecurity 2.9.14 with OWASP CRS 4.22.0
  - IGVM 0.4.0
  - Intel QPL 1.9.0
  - libzpc 2.0.1 (IBM Z)
...
- Security updates:
  - OpenSSL 3.5.7
  - OpenSSH 9.9p1-30
  - SELinux-policy 42.1.27 (SELinux userspace 3.11)
  - SSSD 2.13.1
  - crypto-policies 20260811
  - audit 4.2.1
  - fapolicyd 2.0.1
...
- Other updated components:
  ...
  - Mesa 26.1.1
  - Cockpit 366
  - image-builder 79.0.0
  - rhel-system-roles 2.4.2
  - Pacemaker 3.0.3
  - Valkey 8.1.6

### Deviations from RHEL 10

This is a cumulative list of how AlmaLinux OS 10 differs from RHEL 10. Most entries were introduced in earlier 10.x releases and are repeated here for reference — they are not new in 10.3.

- Added Btrfs support, including ability to boot from Btrfs volume.
- Added x86-64-v2 architecture.
  ...
  - We also rebuild EPEL for x86-64-v2 users. To take advantage of this work and learn more about it, find more information on the AlmaLinux blog: [x86_64_v2 EPEL for the Enterprise Linux 10 World](https://almalinux.org/blog/2025-05-13-epel-10-kitten-v2/)
- Added 32-bit (i686) userspace support (since AlmaLinux 10.2).
  - RHEL 10 ships no 32-bit userspace at all. AlmaLinux's i686 packages cover use cases such as building software that is only available for i686, running CI pipelines that depend on specific 32-bit glibc versions, and containerized workloads requiring a full 32-bit distribution environment. There is still no bootable i686 installer and no 32-bit kernel — userspace only. Packages are published on `vault.almalinux.org`, and official container images are runnable via `docker run --platform linux/386`. The i686 stream is maintained alongside the other AlmaLinux 10 architectures through 2035. For background, see the announcement post for [AlmaLinux Kitten 10 i686](https://almalinux.org/blog/2026-04-16-almalinux-kitten-10-i686/).
...
Upgrade the almalinux-release packages to the 10.3-0.13 version from the beta repositories:

```bash
$ dnf install -y https://vault.almalinux.org/almalinux-{release,repos,gpg-keys}-latest-10-beta.x86_64.rpm
$ dnf clean all
```
...
`AlmaLinux-10.3-beta-1-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-10.3-beta-1-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-10.3-beta-1-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 10.3-beta/isos/$arch/ directory, for example:

```bash
$ wget https://repo.almalinux.org/almalinux/10.3-beta/isos/x86_64/AlmaLinux-10.3-beta-1-x86_64-boot.iso
```
...
```bash
$ wget https://repo.almalinux.org/almalinux/10.3-beta/isos/x86_64/CHECKSUM
...
$ sha256sum AlmaLinux-10.3-beta-1-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-10.3-beta-1-x86_64-boot.iso'
```

If you decide to use the `AlmaLinux-10.3-beta-1-x86_64-boot.iso` image, you will need to provide the `10.3-beta/BaseOS/x86_64/kickstart/` repository from the selected mirror as the Installation Source if the installer will not be able to find closest mirror for some reason.
...
| insights-core                            | RHEL specific package to connect to upstream vendor resources            |
...
| rhc-playbook-verifier                    | RHEL specific package to connect to upstream vendor resources            |
