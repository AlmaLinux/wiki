...
# AlmaLinux 9.8 Release Notes

These are the release notes for AlmaLinux OS 9.8 for all supported architectures:
...
Codename: Olive Jaguar.

Distributed kernel version: 5.14.0-687.5.3.el9_8.
...
You can reference the upstream [RHEL 9.8 release notes](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/9.8_release_notes/index) for a full list of changes in this release.
...
`AlmaLinux-9.8-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-9.8-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-9.8-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 9.8/isos/$arch/ directory, for example:

```bash
$ wget https://repo.almalinux.org/almalinux/9.8/isos/x86_64/AlmaLinux-9.8-x86_64-boot.iso
```
...
```bash
$ wget https://repo.almalinux.org/almalinux/9.8/isos/x86_64/CHECKSUM
...
$ sha256sum AlmaLinux-9.8-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-9.8-x86_64-boot.iso'
```

If you decide to use the `AlmaLinux-9.8-x86_64-boot.iso` image, you may need to provide the `9.8/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the Installation Source in the event that the installer is not able to find the closest mirror for some reason.

There are no extra Installation Sources required if you decided to go with either the minimal or DVD ISO images.

## Upgrade Instructions

To update your AlmaLinux OS 9 to 9.8 run:
...
To update your AlmaLinux 0S 9.8 beta to 9.8 run:

```bash
$ rpm -i https://repo.almalinux.org/almalinux/9.8/BaseOS/x86_64/os/Packages/almalinux-repos-9.8-1.el9.x86_64.rpm --replacefiles
$ dnf upgrade -y
```
