...
You can reference the upstream [RHEL 9.5 release notes](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/9.5_release_notes/index) for a full list of changes in this release.

## Installation instructions

There are three installation ISO images available:

`AlmaLinux-9.5-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-9.5-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-9.5-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 9.5/isos/$arch/ directory, for example:

```bash
$ wget https://vault.almalinux.org/9.5/isos/x86_64/AlmaLinux-9.5-x86_64-boot.iso
```
...
```bash
$ wget https://vault.almalinux.org/9.5/isos/x86_64/CHECKSUM

# we are looking for “Good signature”
$ gpg --verify CHECKSUM
gpg: Signature made Wed 13 Nov 2024 05:00:46 AM EST
...
$ sha256sum AlmaLinux-9.5-x86_64-boot.iso
3038fb71a29d33c3c93117bd8f4c3f612cb152dce057c666b6b11dfa793fb65c  AlmaLinux-9.5-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-9.5-x86_64-boot.iso'
SHA256 (AlmaLinux-9.5-x86_64-boot.iso) = 3038fb71a29d33c3c93117bd8f4c3f612cb152dce057c666b6b11dfa793fb65c
```

If you decide to use the `AlmaLinux-9.5-x86_64-boot.iso` image, you may need to provide the `9.5/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the Installation Source in the event that the installer is not able to find the closest mirror for some reason.
...
To update your AlmaLinux 0S 9.5 beta to 9.5 run:

```bash
$ rpm -i https://vault.almalinux.org/9.5/BaseOS/x86_64/os/Packages/almalinux-repos-9.5-1.el9.x86_64.rpm --replacefiles
$ dnf upgrade -y
```
