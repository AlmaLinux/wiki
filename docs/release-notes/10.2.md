...
These are the release notes for AlmaLinux OS 10.2 for all supported architectures:
...
Codename: Lavender Lion.

Distributed kernel version: 6.12.0-211.7.3.el10_2.
...
Shim 16.1 bootloader binaries for x86_64 are now dual-signed with both the Microsoft 2011 and 2023 UEFI Secure Boot certificates, see [UEFI Secure Boot: Microsoft 2023 Certificate Transition](/documentation/secure-boot-2023-certificates) for details.
...
You can reference the upstream [RHEL 10.2 release notes](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/10.2_release_notes/index) for a full list of changes in this release.
...
- Added 32-bit (i686) userspace support.
  - AlmaLinux 10.2 ships i686 userspace packages. This enables use cases such as building software that is only available for i686, running CI pipelines that depend on specific 32-bit glibc versions, and containerized workloads requiring a full 32-bit distribution environment. Note that there is no bootable i686 installer and no 32-bit kernel — only userspace. Packages are published on `vault.almalinux.org`. Official Docker images runnable via `docker run --platform linux/386` are published alongside the AlmaLinux 10.2 stable release. The i686 stream will be maintained alongside the other AlmaLinux 10 architectures through 2035. For background, see the announcement post for [AlmaLinux Kitten 10 i686](https://almalinux.org/blog/2026-04-16-almalinux-kitten-10-i686/).
...
- CRB repository is enabled by default.
...
`AlmaLinux-10.2-$arch-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-10.2-$arch-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-10.2-$arch-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 10.2/isos/$arch/ directory, for example:

```bash
$ wget https://repo.almalinux.org/almalinux/10.2/isos/$(uname -m)/AlmaLinux-10.2-$(uname -m)-boot.iso
```
...
```bash
$ wget https://repo.almalinux.org/almalinux/10.2/isos/$(uname -m)/CHECKSUM
...
$ sha256sum AlmaLinux-10.2-$(uname -m)-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E "SHA256.*AlmaLinux-10.2-$(uname -m)-boot.iso" || echo "Warning: Checksum does not match"
```

If you decide to use the `AlmaLinux-10.2-x86_64-boot.iso` image, you may need to provide the `10.2/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the _Installation Source_ in the event that the installer is not able to find the closest mirror for some reason.

There are no extra _Installation Sources_ required if you decided to go with either the minimal or DVD ISO images.

## Upgrade Instructions

To update your AlmaLinux OS 10 to 10.2 run:
...
To update your AlmaLinux OS 10.2 beta to 10.2 stable run:
