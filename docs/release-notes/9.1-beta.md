...
```bash
$ wget https://vault.almalinux.org/9.1-beta/isos/x86_64/AlmaLinux-9.1-beta-1-x86_64-boot.iso
```
...
```bash
$ wget https://vault.almalinux.org/9.1-beta/isos/x86_64/CHECKSUM
...
```bash
$ dnf install -y https://vault.almalinux.org/almalinux-release-latest-9-beta.x86_64.rpm
$ dnf clean all
```
