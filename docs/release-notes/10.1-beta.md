...
# AlmaLinux 10.1 Beta Release Notes

These are the release notes for AlmaLinux OS 10.1 Beta for all supported architectures:
...
Distributed kernel version: 6.12.0-124.el10.
...
- The following device drivers were modified to re-add PCI IDs for hardware that was previously disabled in our upstream:
  _ **aacraid** - Dell PERC2, 2/Si, 3/Si, 3/Di, Adaptec Advanced Raid Products, HP NetRAID-4M, IBM ServeRAID & ICP SCSI
  _ **be2iscsi** - Emulex OneConnectOpen-iSCSI for BladeEngine 2 and 3 adapters
  _ **be2net** - Emulex BladeEngine 2 and 3 adapters _
  _ **hpsa** - HP Smart Array Controller
  _ **lpfc** - Emulex LightPulse Fibre Channel SCSI
  _ **megaraid_sas** - Broadcom MegaRAID SAS
  _ **mlx4_core** - Mellanox Gen2, ConnectX-2, ConnectX-3 adapters
  _ **mpt3sas** - LSI MPT Fusion SAS 3.0
  _ **mptsas** - Fusion MPT SAS Host
  _ **qla2xxx** - QLogic Fibre Channel HBA
  _ **qla4xxx** - QLogic iSCSI HBA
...
Upgrade the almalinux-release packages to the 10.1-11 version from the beta repositories:
...
`AlmaLinux-10.1-beta-1-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-10.1-beta-1-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-10.1-beta-1-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 10.1-beta/isos/$arch/ directory, for example:
...
```bash
$ wget https://repo.almalinux.org/almalinux/10.1-beta/isos/x86_64/CHECKSUM

# we are looking for “Good signature”
$ gpg --verify CHECKSUM
gpg: Signature made Mon 20 Oct 2025 11:37:31 AM EDT using RSA key ID C2A1E572
...
$ sha256sum AlmaLinux-10.1-beta-1-x86_64-boot.iso
151633911cbbb79f290e7f0562145c495ac7df8a0f6e61f8976f41ad838df52a  AlmaLinux-10.1-beta-1-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-10.1-beta-1-x86_64-boot.iso'
SHA256 (AlmaLinux-10.1-beta-1-x86_64-boot.iso) = 151633911cbbb79f290e7f0562145c495ac7df8a0f6e61f8976f41ad838df52a
```

If you decide to use the `AlmaLinux-10.1-beta-1-x86_64-boot.iso` image, you will need to provide the `10.1-beta/BaseOS/x86_64/kickstart/` repository from the selected mirror as the Installation Source if the installer will not be able to find closest mirror for some reason.
