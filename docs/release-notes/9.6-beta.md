...
# AlmaLinux 9.6 Beta Release Notes

These are the release notes for AlmaLinux OS 9.6 Beta for all supported architectures:
...
Distributed kernel version: 5.14.0-570.5.1.el9_6.
...
Upgrade the almalinux-release packages to the 9.6-0.0.el9 version from the beta repositories:
...
`AlmaLinux-9.6-beta-1-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-9.6-beta-1-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-9.6-beta-1-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 9.6-beta/isos/$arch/ directory, for example:

```bash
$ wget https://repo.almalinux.org/almalinux/9.6-beta/isos/x86_64/AlmaLinux-9.6-beta-1-x86_64-boot.iso
```
...
```bash
$ wget https://repo.almalinux.org/almalinux/9.6-beta/isos/x86_64/CHECKSUM

# we are looking for “Good signature”
$ gpg --verify CHECKSUM
gpg: Signature made Thu 03 Apr 2025 04:18:42 AM EDT
...
$ sha256sum AlmaLinux-9.6-beta-1-x86_64-boot.iso
b38d8e3875e9c9b89fc577e786331dfa262f5a39b286d37dcd5b6c23e8aee117  AlmaLinux-9.6-beta-1-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-9.6-beta-1-x86_64-boot.iso'
SHA256 (AlmaLinux-9.6-beta-1-x86_64-boot.iso) = b38d8e3875e9c9b89fc577e786331dfa262f5a39b286d37dcd5b6c23e8aee117
```

If you decide to use the `AlmaLinux-9.6-beta-1-x86_64-boot.iso` image, you may need to provide the `https://repo.almalinux.org/vault/9.6-beta/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the Installation Source in the event that the installer is not able to find the closest mirror for some reason.
