...
# AlmaLinux 9.9 Beta Release Notes

These are the release notes for AlmaLinux OS 9.9 Beta for all supported architectures:
...
Codename: Chartreuse Bobcat

Distributed kernel version: 5.14.0-741.el9
...
**GCC Toolset 16 arrives in Enterprise Linux 9**, bringing gcc 16.2.1 and binutils 2.46 alongside the existing GCC Toolset 15. LLVM and Rust toolsets move to 22.1.8 and 1.97.1 respectively.

**fapolicyd moves from 1.4.5 to 2.0.1**, a major version rather than a point update. Since the file access policy daemon sits in the execution path of every binary on a hardened system, existing rules and trust databases are worth re-testing before this reaches production.

**authselect no longer merges groups by default.** Group merging in `nsswitch.conf`, introduced in AlmaLinux 9.8, is now opt-in via the `with-group-merging` feature. Systems that rely on local and remote groups being merged need the feature enabled explicitly after the update.

- Compiler updates:
  - GCC Toolset 16 (gcc 16.2.1, binutils 2.46)
  - LLVM-Toolset 22.1.8
  - Rust-Toolset 1.97.1
...
- Security updates:
  - SELinux-policy 38.1.86
  - SSSD 2.9.9
  - crypto-policies 20260714
  - audit 3.1.5-9
  - fapolicyd 2.0.1
  - authselect 1.2.7
  - Keylime 7.14.3
...
- AlmaLinux OS continues to carry an earlier backport of the upstream kernel fix [d919a1e79bac](https://github.com/torvalds/linux/commit/d919a1e79bac890421537cf02ae773007bf55e6b) ("proc: fix a dentry lock race between release_task and lookup"), which addresses excessive CPU consumption by `systemd` and `ps` during task cleanup. AlmaLinux originally submitted this fix to the CentOS Stream 9 kernel via [MR !7769](https://gitlab.com/redhat/centos-stream/src/kernel/centos-stream-9/-/merge_requests/7769), but its inclusion upstream was postponed. The [AlmaLinux Engineering Steering Committee (ALESCo)](https://almalinux.org/alesco/) voted to include it in AlmaLinux 9.8 ahead of RHEL, and the 9.9 kernel still applies it, as RHEL 9.9 has not picked the fix up.

## Upgrade Instructions

**Please do not use these update instructions on production machines unless you don't mind if something breaks** ;)

Upgrade the almalinux-release packages to the 9.9-0.9 version from the beta repositories:

```bash
$ dnf install -y https://vault.almalinux.org/almalinux-{release,repos,gpg-keys}-latest-9-beta.x86_64.rpm
$ dnf clean all
```
...
`AlmaLinux-9.9-beta-1-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-9.9-beta-1-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-9.9-beta-1-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 9.9-beta/isos/$arch/ directory, for example:

```bash
$ wget https://vault.almalinux.org/9.9-beta/isos/x86_64/AlmaLinux-9.9-beta-1-x86_64-boot.iso
```
...
```bash
$ wget https://vault.almalinux.org/9.9-beta/isos/x86_64/CHECKSUM
...
$ sha256sum AlmaLinux-9.9-beta-1-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-9.9-beta-1-x86_64-boot.iso'
```

If you decide to use the `AlmaLinux-9.9-beta-1-x86_64-boot.iso` image, you may need to provide the `https://repo.almalinux.org/vault/9.9-beta/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the Installation Source in the event that the installer is not able to find the closest mirror for some reason.
