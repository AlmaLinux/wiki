...
# AlmaLinux 10.2 Beta Release Notes

These are the release notes for AlmaLinux OS 10.2 Beta for all supported architectures:
...
Distributed kernel version: 6.12.0-211.7.1.el10_2
...
- Added 32-bit (i686) userspace support.
  - AlmaLinux 10.2-beta ships i686 userspace packages. This enables use cases such as building software that is only available for i686, running CI pipelines that depend on specific 32-bit glibc versions, and containerized workloads requiring a full 32-bit distribution environment. Note that there is no bootable i686 installer and no 32-bit kernel — only userspace. Packages are published on `vault.almalinux.org`. Official Docker images runnable via `docker run --platform linux/386` will be published alongside the AlmaLinux 10.2 stable release. The i686 stream will be maintained alongside the other AlmaLinux 10 architectures through 2035. For background, see the announcement post for [AlmaLinux Kitten 10 i686](https://almalinux.org/blog/2026-04-16-almalinux-kitten-10-i686/).
...
Upgrade the almalinux-release packages to the 10.2-0.4 version from the beta repositories:
...
`AlmaLinux-10.2-beta-1-x86_64-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-10.2-beta-1-x86_64-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-10.2-beta-1-x86_64-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 10.2-beta/isos/$arch/ directory, for example:

```bash
$ wget https://repo.almalinux.org/almalinux/10.2-beta/isos/x86_64/AlmaLinux-10.2-beta-1-x86_64-boot.iso
```
...
```bash
$ wget https://repo.almalinux.org/almalinux/10.2-beta/isos/x86_64/CHECKSUM
...
$ sha256sum AlmaLinux-10.2-beta-1-x86_64-boot.iso

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E 'SHA256.*AlmaLinux-10.2-beta-1-x86_64-boot.iso'
```

If you decide to use the `AlmaLinux-10.2-beta-1-x86_64-boot.iso` image, you will need to provide the `10.2-beta/BaseOS/x86_64/kickstart/` repository from the selected mirror as the Installation Source if the installer will not be able to find closest mirror for some reason.
