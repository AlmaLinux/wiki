...
# AlmaLinux 10.1 Release Notes

These are the release notes for AlmaLinux OS 10.1 for all supported architectures:
...
Distributed kernel version: 6.12.0-124.8.1.el10_1.
...
- Compiler updates:
  ...
  - GCC Toolset 15
    - GCC 15.1
    - Binutils 2.44
    - _Note that `Annobin` and `dwz` are not provided in GCC Toolset starting with version 15._
  - Go Toolset 1.24
  - LLVM Toolset 20.1.8
  - Rust Toolset 1.88.0
  - glibc 2.39
...
- Security updates:
  ...
  - This release introduces support for RPMv6 signatures. This new format enables multiple signatures in an RPM package. You can use Sequoia PGP tools to sign RPM packages with PQC algorithms and create or verify OpenPGPv6 signatures.
...
You can reference the upstream [RHEL 10.1 release notes](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/10.1_release_notes/index) for a full list of changes in this release.
...
- The following device drivers were modified to re-add PCI IDs for hardware that was previously disabled in our upstream:
  ...
  - **be2net** - Emulex BladeEngine 2 and 3 adapters \_
...
`AlmaLinux-10.1-$arch-boot.iso` - a single network installation CD image that downloads packages over the Internet.

`AlmaLinux-10.1-$arch-minimal.iso` - a minimal self-containing DVD image that makes possible offline installation.

`AlmaLinux-10.1-$arch-dvd.iso` - a full installation DVD image that contains mostly all AlmaLinux packages.

Download a suitable ISO image from the 10.1/isos/$arch/ directory, for example:
...
$ gpg --verify CHECKSUM
gpg: Signature made Wed 19 Nov 2025 02:37:44 PM EST using RSA key ID C2A1E572
...
$ sha256sum AlmaLinux-10.1-$(uname -m)-boot.iso
68a9e14fa252c817d11a3c80306e5a21b2db37c21173fd3f52a9eb6ced25a4a0  AlmaLinux-10.1-x86_64-boot.iso # x86_64 checksum, yours may differ

# compare it with expected checksum, it should be the same
$ cat CHECKSUM | grep -E "SHA256.*AlmaLinux-10.1-$(uname -m)-boot.iso" || echo "Warning: Checksum does not match"
SHA256 (AlmaLinux-10.1-x86_64-boot.iso) = 68a9e14fa252c817d11a3c80306e5a21b2db37c21173fd3f52a9eb6ced25a4a0 # x86_64 checksum, yours may differ
```

If you decide to use the `AlmaLinux-10.1-x86_64-boot.iso` image, you may need to provide the `10.1/BaseOS/x86_64/kickstart/` URL repository from the selected mirror as the _Installation Source_ in the event that the installer is not able to find the closest mirror for some reason.

There are no extra _Installation Sources_ required if you decided to go with either the minimal or DVD ISO images.

## 10.1 Beta Testers

To update your AlmaLinux 0S 10.1 beta to 10.1 stable run:
