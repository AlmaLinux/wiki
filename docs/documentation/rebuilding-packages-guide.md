---
title: Rebuilding AlmaLinux's packages Guide
---

# Rebuilding AlmaLinux's packages

This guide contains step-by-step instructions on how to rebuild AlmaLinux's packages for different architectures. It also includes some background information about the mock tool and some examples. These details can be helpful for the rebuilding packages process.

## About `mock`

[Mock](https://rpm-software-management.github.io/mock/) is a tool for building RPM packages. It allows you to build packages for multiple architectures and different versions of RPM-based systems. It's possible to use mock to build AlmaLinux OS packages as well.

This guide contains steps on how to rebuild AlmaLinux packages for different architectures.
...
![image](/images/rebuilding-packages-guide_etc-mock.png)

:::warning
Mock doesn't have a config file to build i686 packages. So you have to create a config yourselves to make that possible.
:::

Mock rebuilds RPMs from source RPMs which you need to download first. All packages are available at repo.almalinux.org. 
...
## Package rebuilding instruction if changes are needed
...
:::tip
Get more information about [AlmaLinux supports EPEL and other repositories](https://wiki.almalinux.org/repos/Extras.html) on AlmaLinux wiki.
:::
...
* Run the following command to be added to the mock group. That is necessary as you are going to use mock to rebuild packages:

```
sudo usermod -a -G mock username
```

* Once you've installed mock and put the user in the mock group, you need to download a package you want to rebuild from [repo.almalinux.org](https://repo.almalinux.org/). Use the `wget` command with the URL of the package. The downloaded package will appear in your home directory unless you specify.
...
![images](/images/rebuilding-packages-guide_rpmbuild.png)

Make sure you chose the right rpmbuild option as there are several of them: 
| Option | Usage |
|---|---|
| -ba | Build all, both a binary and source RPM |
|-bb | Build a binary RPM |
|-bc | Build the program but do not make the full RPM, stopping just after the build section |
...
* Switch to the SRPMS directory to rebuild the package. The next step depends on what architecture you are rebuilding packages.

### x86_64

Use the `almalinux-8-x86_64.cfg` file that mock already has to rebuild packages.

Run the following command to rebuild the package:
...
Note, mock doesn't have a specified config file for AlmaLinux to build i686 packages. So you have to download this [config file](#config-file-for-i686). Next, we need to put that config to the `/etc/mock/` directory. It is possible to build i686 now.
...
## Rebuilding insctruction without changes 

In case you need to rebuild a package without any changes, follow mentioned steps:
* [Install](#package-rebuilding-instruction-if-changes-are-needed) `epel-release`, `mock` and `add a user` to the mock group.
...
* Run `mock -r "configname" --rebuild fullpackagename.src.rpm` to rebuild the package.

:::tip
"configname" is the name of a configuration file from `/etc/mock/` without the `/etc/mock/` path prefix and the `.cfg` suffix. [Check the information above](#x86-64) to see examples.
:::

Builds are done in the `/var/lib/mock` directory. You can see the built packages in this directory.

![image](/images/rebuilding-packages-guide_results.png)
